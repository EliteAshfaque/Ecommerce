...
export async function getAIRecommendation(req, res, userPrompt, products) {
  ...
  try {
    const geminiPrompt = `
      ...
      Only return the matching products in JSON format. Do not include any markdown formatting, just the raw JSON array.
    `;
    ...
    // Fixed typo: 'condidates' -> 'candidates'
    ...
    // Completed the cut-off catch block
