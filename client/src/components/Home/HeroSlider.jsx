...
const HeroSlider = ({ banners = [] }) => {
  ...
  // Every active campaign advances automatically; the banners themselves come from storefront_banners.
  useEffect(() => { if (slides.length < 2) return undefined; const timer = setInterval(next, 6500); return () => clearInterval(timer); }, [next, slides.length]);
  ...
  return <section className="relative isolate min-h-[510px] overflow-hidden bg-[#151331] pt-16 text-white md:min-h-[550px]">
    ...
    <div className="relative mx-auto flex min-h-[calc(510px-4rem)] max-w-7xl items-end px-6 pb-11 pt-20 md:min-h-[calc(550px-4rem)] md:px-8 md:pb-12 md:pt-20"><div className="grid w-full items-end gap-10 lg:grid-cols-12"><div className="animate-fade-in-up max-w-2xl lg:col-span-7" key={banner.id}><p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.28em] text-violet-200 md:text-[11px]"><Sparkles className="h-3.5 w-3.5" /> {banner.eyebrow}</p><h1 className="mt-4 max-w-xl font-display text-4xl font-semibold leading-[.95] tracking-[-.05em] sm:text-5xl md:text-6xl">{banner.title}</h1><p className="mt-4 max-w-lg text-sm leading-relaxed text-white/72 md:text-base">{banner.description}</p><Link to={banner.cta_url} className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-white px-5 py-3.5 text-[11px] font-bold uppercase tracking-[.18em] text-[#191638] transition hover:-translate-y-0.5 hover:bg-violet-100">{banner.cta_label}<ArrowUpRight className="h-4 w-4" /></Link><div className="mt-7 flex gap-2 lg:hidden">{slides.map((slide, index) => <button key={slide.id} onClick={() => setCurrent(index)} className={`h-1.5 rounded-full transition ${activeIndex === index ? "w-12 bg-white" : "w-5 bg-white/30 hover:bg-white/60"}`} aria-label={`Show ${slide.title}`} />)}</div></div><aside className="hidden border-l border-white/20 pl-7 lg:col-span-5 lg:block"><p className="text-[10px] font-semibold uppercase tracking-[.22em] text-white/50">The promise</p><div className="mt-4 space-y-3">{["AED pricing", "Secure checkout", "UAE delivery"].map((signal) => <p key={signal} className="flex items-center gap-3 text-sm text-white/80"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-white/12"><Check className="h-3.5 w-3.5 text-violet-200" /></span>{signal}</p>)}</div><div className="mt-7 flex gap-2">{slides.map((slide, index) => <button key={slide.id} onClick={() => setCurrent(index)} className={`h-1.5 rounded-full transition ${activeIndex === index ? "w-12 bg-white" : "w-5 bg-white/30 hover:bg-white/60"}`} aria-label={`Show ${slide.title}`} />)}</div></aside></div></div>
